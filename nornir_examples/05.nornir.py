...
def load_vlan(task):
    v = task.run(task=template_file, template="add_vlans.j2", path="./templates")
...
yaml_targets = nr.filter(site="site_a", role="spine")
...
vlan_targets = nr.filter(site="site_a", role="spine")
vlan_targets = nr.filter(role="spine")
